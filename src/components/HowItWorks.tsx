import { MessageSquareText, ShieldCheck, Wallet } from "lucide-react";

const steps = [
  {
    number: "01",
    icon: MessageSquareText,
    title: "Tell us what you want to trade",
    description:
      "Reach out on WhatsApp or the contact form with the coin and amount you want to buy or sell.",
  },
  {
    number: "02",
    icon: ShieldCheck,
    title: "Confirm the rate",
    description:
      "We give you a clear, live rate. No hidden charges, no surprises — you confirm before anything moves.",
  },
  {
    number: "03",
    icon: Wallet,
    title: "Get settled fast",
    description:
      "Send your coin or payment and receive your payout promptly once confirmed on our end.",
  },
];

export default function HowItWorks() {
  return (
    <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20">
      <div className="text-center">
        <span className="inline-flex rounded-full border border-maroon-600/20 bg-maroon-50 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-maroon-700">
          Process
        </span>
        <h2 className="mt-4 font-display text-3xl font-extrabold text-maroon-950 sm:text-4xl">
          How it works
        </h2>
        <p className="mx-auto mt-3 max-w-xl text-maroon-950/60">
          A simple, three-step process built around clarity and speed.
        </p>
      </div>

      <div className="mt-12 grid gap-6 md:grid-cols-3">
        {steps.map((step) => (
          <div
            key={step.title}
            className="group relative overflow-hidden rounded-2xl border border-cream-300 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-gold-300 hover:shadow-md"
          >
            <span className="pointer-events-none absolute right-4 top-2 font-display text-6xl font-black text-maroon-950/[0.04] transition-colors group-hover:text-gold-400/10">
              {step.number}
            </span>
            <div className="relative flex h-12 w-12 items-center justify-center rounded-xl bg-gold-500 text-maroon-950">
              <step.icon size={22} />
            </div>
            <h3 className="relative mt-4 font-display text-lg font-bold text-maroon-950">
              {step.title}
            </h3>
            <p className="relative mt-2 text-sm text-maroon-950/60">
              {step.description}
            </p>
          </div>
        ))}
      </div>
    </section>
  );
}
