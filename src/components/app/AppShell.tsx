...
const ACTIVE_STATUSES: OrderDTO["status"][] = [
  "created",
  "awaiting_payment",
  "payment_marked",
  "payment_confirmed",
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    function poll() {
      api
        .listMyOrders()
        .then((orders) => {
          if (!cancelled) setActiveOrders(orders.filter((o) => ACTIVE_STATUSES.includes(o.status)));
        })
        .catch(() => {});
    }
    poll();
  ...
  return (
    <div className="mx-auto flex min-h-full w-full max-w-md flex-1 flex-col bg-cream-100">
      ...
      {latestActive && !onTradePage && (
        <Link
          href={`/trade/${latestActive.id}`}
          className="fixed inset-x-4 bottom-24 z-30 mx-auto flex max-w-[27rem] items-center justify-between rounded-2xl bg-maroon-900 px-4 py-3 text-white shadow-xl"
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-white/60">
                {latestActive.amount} {latestActive.asset} · tap to continue
              </p>
            </div>
          </div>
          <span className="rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold">Open</span>
        </Link>
      )}
